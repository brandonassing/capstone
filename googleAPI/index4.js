...
var once = false
var i = 900 // SCRIPTS PICKS UP AT THIS INDEX
var jsonArray;
var lastLink = false
// global script variables ----------
...
var dest = "./downloads4/" + unique + ".mp3"
...
async function convertMP3toWAV(dest, callId,lastLink, metadata,unique, cb) {
  ...
  var mp3File = dest
  var outputDestiation = './wavFiles4/' + unique + ".wav"
...
async function upload2GoogleBucket(localUrl, callId,lastLink, metadata, unique, cb2){
  ...
  const bucketName = "formatedwavfiles4"
...
async function googleSpeech2Text(name,callId, lastLink, metadata,unique, cb3) {
  ...
  if(lastLink){
    ...
    fs.writeFile("Transcriptions900-1199.csv", csv, function(err, data) {
