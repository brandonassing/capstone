...
const csvFilePath='./csv/merged.csv'
...
var once = false
var i = 1500 // SCRIPTS PICKS UP AT THIS INDEX
var jsonArray;
var lastLink = false
// global script variables ----------
...
var dest = "./downloads6/" + unique + ".mp3"
...
async function convertMP3toWAV(dest, callId,lastLink, metadata,unique, cb) {
  ...
  var mp3File = dest
  var outputDestiation = './wavFiles6/' + unique + ".wav"
...
async function upload2GoogleBucket(localUrl, callId,lastLink, metadata,unique, cb2){
  ...
  const bucketName = "formatedwavfiles6"
...
async function googleSpeech2Text(name,callId, lastLink, metadata,unique, cb3) {
  ...
  if(lastLink){
    ...
    fs.writeFile("Transcriptions1500-1799.csv", csv, function(err, data) {
